@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex flex-col items-center">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between sm:flex-col">
            ...
            <div>
                <span class="relative z-0 inline-flex rounded-md shadow-lg">
                    {{-- Previous Page Link --}}
                    @if ($paginator->onFirstPage())
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium font-bold leading-5 bg-white border border-gray-300 cursor-default text-pizza-orange">{{ $page }}</span>
                                    </span>
                                @else
