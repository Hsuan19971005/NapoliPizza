<x-online-order-layout>
    ...
    <div class="py-5">
        ...
        <section class="">
            <form action="" method="post" class="max-w-xl m-2 mx-auto text-xl text-black">
                @csrf
                @foreach ($cartItems as $item)
                <div class="mb-1 overflow-x-auto border-2 food-card border-stone-500">
                    <div class="flex px-3 py-2 bg-white">餐點：<span class="ml-auto">{{ $item['product']->name ?? 'name' }}</span></div>
                    <div class="flex px-3 py-2 bg-gray-100 ">數量：<span class="ml-auto">{{ $item['quantity'] ?? 'quantity' }}</span></div>
                    <div class="flex px-3 py-2 bg-white">小計：<span class="ml-auto unit-price" data-unit-price="{{ $item['product']->price }}" data-quantity="{{ $item['quantity'] }}">${{ floor($item['product']->price * $item['quantity']) }}</span></div>
                ...
                <div class="flex flex-col sm:flex-row">
                    <a href="{{ route('order.create') }}" class="flex items-center justify-center p-2 mb-3 text-lg bg-purple-300 rounded-md sm:w-full sm:mb-0 sm:mr-2">
