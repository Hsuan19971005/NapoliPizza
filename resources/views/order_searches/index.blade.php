<x-online-order-layout>
    ...
    <div class="py-5">
        <h2 class="flex items-center justify-center mb-3 text-3xl font-bold text-black">
            ...
            訂單查詢</h2>
        <form action="" class="max-w-sm px-5 py-4 mx-auto text-lg text-black rounded shadow bg-gray-50">
            <div class="mb-7 form-group">
                <label for="" class="block w-full h-8">訂單編號：</label>
                <input type="text" name="serial_number" class="block w-full rounded h-9" placeholder="請輸入訂單編號">
                @error('serial_number')
                    <label>{{ $message }}</label>
                @enderror
            </div>
            <div class="flex items-end h-10 mb-1 form-group">
                <label for="">驗證碼：</label>
                <span class="h-full mr-3"><img id="captcha-img" src="{{ captcha_src() }}" alt="captcha-img" class="h-full"></span>
                <button type="button" id="reload" class="h-full text-3xl text-white bg-red-500 rounded aspect-square hover:bg-red-600">&#x21bb;</button>
            </div>

            <div class="bg-purple-400 mb-7 form-group">
